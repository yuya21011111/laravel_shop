...
<div class="modal micromodal-slide" id="{{ $modal }}" aria-hidden="true">
          ...
          <div class="flex flex-wrap">
            ...
            <div class="w-1/4 p-2 md:p-4">
             ...
             <img class="image" data-id="{{ $name }}_{{ $image->id }}"
                        ...
                        data-path="{{ asset('products/') }}"
                        data-modal="{{ $modal }}"
                        src="{{ asset('products/' . $image->filename)}}">
  ...
  <div class="flex justify-around items-center mb-4">
    <a class="px-2 py-1 bg-green-400 text-white font-semibold rounded hover:bg-green-500" data-micromodal-trigger="{{ $modal }}" href='javascript:;'>ファイルを選択</a>
    <div class="w-1/4">
    <img id="{{ $name }}_thumbnail" @if($cImage) src="{{ asset('products/' . $cImage)}}" @else src="" @endif>
    </div>
  </div>
